...
* Danach können unterschiedliche Aktionen, je nach Typ des Elements mit diesem durchgeführt werden.
  * Für Eingaben nutzern wir hier die Methode `fill()`.
  * Weitere Informationen zu [fill](https://playwright.dev/docs/input#text-input)
  * Weitere Informationen zu Input-Möglichkeiten: [Input](https://playwright.dev/docs/input)

````typescript
When('ich als erste Zahl drei eingebe', async function (this: ICustomWorld) {
  const page = this.page!;
  await page.locator('input#number1Field').fill('3');
});

When('ich als zweite Zahl vier eingebe', async function (this: ICustomWorld) {
  const page = this.page!;
  await page.locator('input#number2Field').fill('4');
});
````
